...
static bool isUpdatedByDominatingStore(Operation *op, mlir::FuncOp f) {
  ...
  while (!worklist.empty()) {
    ...
    if (mlir::AffineLoadOp loadOp = dyn_cast<mlir::AffineLoadOp>(currOp)) {
        mlir::Value memref = loadOp.memref();
...
static void getScopStmtOps(Operation *writeOp, llvm::SetVector<Operation *> &ops,
...
void hcl::registerToOpenScopExtractTranslation() {
  static TranslateFromMLIRRegistration toScopStmt("extract-scop-stmt", extractOpenScop, [&](DialectRegistry &registry){
