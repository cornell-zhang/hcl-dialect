...
def main(P=16, Q=22, R=18, S=24):
    f1, f2, f3 = top_2mm(P=P, Q=Q, R=R, S=S)
    A = np.random.randint(10, (P, Q)).astype(np.float32)
    B = np.random.randint(10, (Q, R)).astype(np.float32)
    C = np.random.randint(10, (R, S)).astype(np.float32)
    D = np.random.randint(10, (P, S)).astype(np.float32)
